...
def generate_response():
    ...
    # Split the GPT response into sections
    sections = re.split(r'^### (.*?)\n', answer, flags=re.MULTILINE)

    # First part is always the intro (possibly blank), followed by alternating [title, content, title, content...]
    structured = {}
    current_title = None
    for i, part in enumerate(sections):
        if i == 0 and part.strip():
            ...
            structured[current_title] = part.strip()

    # Add structured sections to the Word doc
    for title in ["Enquirer Reply", "Action Sheet", "Policy Notes"]:
        if title in structured:
            ...
            if title == "Action Sheet":
                ...
                # Default paragraph with markdown styling
